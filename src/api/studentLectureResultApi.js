...
export const getStudentLectureResultsByLecture = async lectureId => {
  try {
    ...
    console.log('API 요청 URL:', `${API_BASE_URL}/student-lecture-results/lectures/${lectureId}`);

    const token = localStorage.getItem('token');
    console.log('토큰 확인:', token ? '토큰 존재' : '토큰 없음');

    const response = await fetch(`${API_BASE_URL}/student-lecture-results/lectures/${lectureId}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        ...(token && { Authorization: `Bearer ${token}` }),
...
export const getStudentLectureResultByStudentAndLecture = async (studentId, lectureId) => {
  try {
    console.log('학생-강의별 결과 조회 API 요청:', { studentId, lectureId });
    console.log('API 요청 URL:', `${API_BASE_URL}/student-lecture-results/students/${studentId}/lectures/${lectureId}`);

    const token = localStorage.getItem('token');
    console.log('토큰 확인:', token ? '토큰 존재' : '토큰 없음');

    const response = await fetch(`${API_BASE_URL}/student-lecture-results/students/${studentId}/lectures/${lectureId}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        ...(token && { Authorization: `Bearer ${token}` }),
